...
package Lesson.Chapter_8.bookpackext;// Данный класс принадлежит пакету bookpackext

/*
320 Java: руководство для начинающих, 7-е издание
...
class ExtBook extends Lesson.Chapter_8.bookpack.Book {
  ...
  public String getAuthor ()  {  return author;  }  //◄-----Доступ к членам класса Book разрешен 
                                                    //для подклассов
...
/*
 * Обратите внимание на код класса ExtBook. 
 * В связи с тем что класс ExtBookявляется подклассом, 
 * производным от класса Book, он  имеет доступ к защищенным членам класса Book.
 * Это правило действует, несмотря на то что класс ExtBook находится в другом пакете.
 * Следовательно, он может обращаться непосредственно к переменным экземпляра
 * title, author и pubDate, что и было использовано при написании методов доступа.
 * В то же время доступ к этим  переменным  экземпляра из класса ProtectDemo запрещен,
 * поскольку класс ProtectDemo не является подклассом, производным от класса Book. 
 * Так, если удалить комментарии в приведенной ниже строке кода, то программа не будет
 * скомпилирована.
 * //    books[0].title = "test title";  // Ошибка:  доступ запрещен!
 */