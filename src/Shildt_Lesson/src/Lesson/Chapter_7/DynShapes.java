...
/**
 * Для того чтобы стало понятнее, насколько эффективным является механизм переопределения методов, 
 * продемонстрируем его применение на примере класса TwoDShape. В приведенных ранее примерах
 *  в каждом классе, наследующем класс TwoDShape, определялся метод area (). Теперь мы знаем,
 *  что в этом случае имеет смысл включить метод area ()  в состав класса TwoDShape, позволить каждому
 *  его подклассу переопределить этот метод и, в частности, реализовать вычисление площади в зависимости 
 * от конкретного типа геометрической фигуры.  Именно такой подход и реализован в приведенном ниже примере программы. 
 * Для удобства в класс TwoDShape добавлено поле name. (Это упрощает написание демонстрационной программы.)
 */
